...
fn transfer_control_proposes_then_accepts() {
    ...
    // Propose transfer ΓÇö pending is set, controller unchanged
    ...
    // Accept ΓÇö control moves to next
...
fn accept_control_by_wrong_address_fails() {
 ...
 feat/recipient-helpers
    ...
    // Intruder tries to accept ΓÇö auth fails because mock_all_auths won't be set
    s.env.set_auths(&[]);
    let result = s.env.try_invoke_contract::<(), Error>(
        &s.client.address,
...
fn cancel_transfer_clears_pending() {
    ...
    // Cancel ΓÇö pending cleared, controller stays
...
fn renounce_control_still_works_in_one_step() {
    ...
    // Renounce ΓÇö immediate and irreversible
 main
